...
const check_notification = async (check_user) => {
    ...
    let near_expire_check_w = role.filter((el) => { return el.application_id == '62ac7d3608889e4f8e123225' })

    //equipment
    let expire_check_e = role.filter((el) => { return el.application_id == '62d443fe2ee0fe85f35b714a' })
    let near_expire_check_e = role.filter((el) => { return el.application_id == '62d444452ee0fe85f35b7157' })
    ...
    //scaff
    let expire_check_s = role.filter((el) => { return el.application_id == '62dd29333eb3c4301b4af56d' })
    let near_expire_check_s = role.filter((el) => { return el.application_id == '62dd29713eb3c4301b4af57b' })
    ...
    if (near_expire_check_w.length > 0 || gas_check.length > 0 || impairment_chek.length > 0 || role.length == 0) {
        await WorkPermit.find({
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                let workingEnd = new Date(element.others.workingEnd)
    ...
    if (expire_check_e.length > 0 || near_expire_check_e.length > 0 || obstruct_check.length > 0 || not_remove_check.length > 0 || role.length == 0) {
        ...
        await Equipment.find({
            ...
            for (let index = 0; index < result.length; index++) {
                ...
                let join_equipment_vehicle = EquipmentVehicle_master.filter(el => { return el.ObstructionID == element.EquipmentID })
